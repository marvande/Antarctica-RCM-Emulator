...
# UNet that adds a channel with the variance over time at each pixel 
class UNetVariance(nn.Module):
    def __init__(
        self, n_channels_x=7, n_channels_z=16, size=32, filter=64, bilinear=False
    ):
        super().__init__()
        self.n_channels_x = n_channels_x*2 # add std for each channel
        ...
        # input  (batch, 7, 32, 32)
        # first conv:
        self.initialconv = InitialConv(self.n_channels_x, 32, 32, self.size)
    ...
    def forward(self, x, z):
        # calculate variance at each pixel over batch: 
        #stdX = x.std(dim = 0).unsqueeze(0) # (1, 7, 32, 32)
        #meanX = x.mean(dim = 0).unsqueeze(0) # (1, 7, 32, 32)
        
        #print(stdX.shape)
        # multiply dim 0 by batch so that can concat on dim 1
        stdX = torch.repeat_interleave(stdX, x.shape[0], dim=0) # (batch, 7, 32, 32)
        print('std')
        print(stdX)
        print(x.shape[0], stdX.shape)
        
        # concatenante variance with input
        # x input  (batch, 7, 32, 32)
        x0 = torch.cat([x, stdX], dim = 1) # (batch, 14, 32, 32)
        print(x0.shape)
        print('x0')
        print(x0)
        x1 = self.initialconv(x0)  # (batch, 32, 32, 32)
        print('x1')
        print(x1)
        # Downsampling
