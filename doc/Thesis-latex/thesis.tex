...
\vspace{4cm}
\begin{center}
    This thesis is dedicated to my grandma Brigitte Dorigo Baumann, my biggest supporter.
\textit{Diese Arbeit ist meiner Grossmutter Brigitte Dorigo Baumann gewidmet, meiner grössten Unterstützerin. Danke liebes Grosi, für all deine Unterstüzung, ich hab dich sehr lieb.}
\end{center}}
\end{comment}
...
I would like to express my special thanks to Christoph Kittel for providing me with the MAR(ACCESS1.3) RCM dataset and his availability to answer my questions. Likewise, I would like to thank Doury et al.'s team for providing me with the code of their RCM-Emulator. 

I would like to thank my proof-readers in order of occurrence but not of importance: Melissa Corboz, Jan Roelof van der Meer. 
...
\begin{abstract}
Surface mass balance (SMB) is a crucial indicator of the stability and evolution of the Antarctic ice sheet (AIS), and it is therefore imperative to have a precise estimation of it. Currently, however, it is challenging to model SMB correctly because of its high variability across space and time and its complex interactivity with atmospheric variables. In this thesis, we developed a machine learning framework that learns to create regional (RCM) SMB predictions from global-scale climate model outputs (GCM). Specifically, we emulated MAR(ACCESS 1.3) SMB values of the Antarctic Peninsula, the fastest warming region of the AIS, which loses mass at significant rates. We built the RCM-emulator using a U-Net with attention mechanisms, depth-wise separable convolutions, and a normalized RMSE loss. In our methodology, to train our emulator, we proposed two frameworks. In the first i.e., the perfect model framework, instead of low-resolution inputs from the GCM, the RCM-emulator received upscaled RCM features at GCM resolution ($\mathrm{\hat{F}_{U}}$). This was designed so that the RCM-emulator learned only the RCM downscaling function, undisturbed by potential RCM-GCM inconsistencies and biases. $\mathrm{\hat{F}_{U}}$ was both evaluated on upscaled RCM and GCM features. In the second framework, we trained the emulator with large-scale features directly sourced from the GCM ($\mathrm{\hat{F}_{G}}$) to evaluate
whether it could learn those RCM-GCM inconsistencies. Our results show that when $\mathrm{\hat{F}_{U}}$ predicts using upscaled RCM features, it can almost perfectly reproduce the high-resolution SMB truth. However, its predictions made using GCM features consistently underestimate it. Interestingly, the predictions of $\mathrm{\hat{F}_{G}}$ show that despite large-scale bias between the GCM and RCM, the RCM-emulator can reproduce the true SMB values successfully, except for very arid regions. Assuming that RCM-GCM inconsistencies exist for a valid reason and that we wish the SMB-emulator to account for them, our results indicate that a perfect model framework might not be needed and that our model can learn them. Finally, our model is a large computational gain compared to an RCM simulation, with training under ten minutes and predictions in a few seconds. 

We conclude that machine learning emulators can be applied to make fast and fine-scaled reproductions of RCM simulations. Therefore, they can be an interesting tool for providing low cost local-scale information in a changing climate. 
...
\begin{itemize}
    ...
    \begin{comment}
       Their results show that their emulator provided a substantial computational gain in computing an RCM, with two hours of training on a GPU and predictions within seconds. Furthermore, when evaluated in the perfect model framework i.e., with large-scale inputs from an upscaled RCM, their emulator could almost perfectly reproduce the original TAS series. The emulator remained robust even when getting GCM inputs, but its performance was poorer because of RCM-GCM large-scale decorrelation issues. Because the authors did not train it on GCM features, the model could not reproduce large-scale inconsistencies between GCMs and RCMs. Doury et al. thus questioned the usability of their RCM-emulator since it missed those inconsistencies~\cite{Doury}. 
        \end{comment}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item For precipitation, the spatial patterns in the UPRCM and GCM are very different, with the RCM being much more precise in its predictions. For example, for the month of November in 2093, the UPRCM shows a local high-intensity precipitation event on the tip of the Peninsula, while the GCM predicts a more vague pattern of lower intensity over the water in the west. 
        \item We also notice a streak pattern on the left boundary of the RCM for all variables, but this is a known problem as the boundaries of climate models are usually imprecise. 
    \end{itemize}
    \item Overall, we found problems in consistency between large and local-scale variables. As mentioned in \autoref{subsec:perfect-model}, these inconsistencies might confuse an Emulator, and this might justify the need for the perfect model framework for training a model. 
\end{itemize}
