...
\begin{itemize}
   \item Target domain: The target domain used for this model is a grid box of $64 \times 64$ at 35 \si{mmWe/day}\si{km} resolution that contains the Antarctic Peninsula in West Antarctica. 
...
\begin{itemize}
    ...
    \item Consistency: Start by evaluating consistency between GCM and UPRCM variables. Why? This way we know how much bias between LR and HR variables the Emulators are dealing with and whether a perfect model framework is necessary. Interesting to know for when UPRCM-Emulator gets GCM inputs and interesting for GCM-Emulator. 
        ...
        \item Feature importance for both Emulators. Why? From a physical point of view, interesting to see which atmospheric variables play the biggest impact on SMB when its content are scrambled. 
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Regarding the wind variables, we suspect that due to differences in high and low-pressure systems, the wind variables show different temporal patterns.
    \end{itemize}
...
To look more closely at how the models adapt to different patterns and intensities of SMB, we plot the predicted time series for four points (Fig.~\ref{fig:points-location}). We chose two points in regions with higher levels of precipitation: point one on the Larsen C ice-shelf that annually oscillates between $-5$ and $5$ \si{[mmWe/day]} and point two on the west coast which reaches low extremes of $-15$ \si{[mmWe/day]} annually. On the other side, point four on the Ronne ice-shelf has a drier climate ($0-1.5$ \si{[mmWe/day]}). Finally, we chose point three on the east coast for its drier weather ($0-4$ \si{[mmWe/day]}) and its SMB pattern that looks less seasonal than the other three points. Time series and annual predictions in Fig.~\ref{fig:timeseries-GCM-UPRCM}. 
\begin{itemize}
    \item For each of these 4 points, $\hat{F}_{U}(UPRCM)$ and $\hat{F}_{G}(GCM)$ come very close to reproducing the temporal patterns of the RCM truth series. 
    \item For points one and two, we see that the seasonality is well captured by all emulators, with high correlation values (close to $1$ for P2). $\hat{F}_{G}(GCM)$ looks like it is even a little bit better than $\hat{F}_{U}(UPRCM)$ for reproducing low drops and high peaks. 
    \item For points three and four, even if they miss a few of the peaks in SMB, $\hat{F}_{U}(UPRCM)$ and $\hat{F}_{G}(GCM)$ capture well the time-series pattern, even when it is less cyclical like for P3. For P4 we notice that the Emulator struggles when faced with multiple close peaks per year and merges them together in one large peak. 
    \item As was already seen in Fig.~\ref{fig:geoplots-GCM-RCM}, $\hat{F}_{U}(GCM)$ struggles to reproduce high intensity values and this is highly noticeable in its time-series. This Emulator's time-series capture the seasonal patterns (reflected in its high correlation) but produces a toned-down version of the RCM truth time series (reflected in its higher RMSE).
    \item This behaviour is also reflected when looking at the annual SMB predictions for all models. $\hat{F}_{U}(UPRCM)$ and $\hat{F}_{G}(GCM)$ come very close to the true annual SMB values, with $\hat{F}_{G}(GCM)$ again a little bit better than $\hat{F}_{U}(UPRCM)$ for almost all years. However, $\hat{F}_{U}(GCM)$ in average always underestimates the truth by half. 
...
\begin{itemize}
    ...
    This first evaluation step shows that if the emulator is still perfectible, in particular when looking at extremes or climate change intensity, it is able to almost perfectly reproduce the spatial structure and daily variability of the near surface temperature in the perfect model world.
    \item Within this learning framework, the emulator can not learn GCM-RCM large-scale inconsistencies, if there should be any. Therefore, when GCM inputs are given to the emulator, the estimated RCM downscaling function is applied to the GCM large scales fields, and any GCM-RCM bias is conserved between the emulated serie and the RCM one.
\end{itemize}
