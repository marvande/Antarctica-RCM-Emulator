...
I would like to thank my proof-readers in order of occurrence but not of importance: Melissa Corboz [a completer]. 

Finally, I would like to thank Prof. Payer from the EPFL for creating the practical latex package used to write this EPFL thesis. The latest version of this package is available \hyperref{http://www.latex-project.org/lppl.txt}{here}.}
...
Surface mass balance is a crucial indicator variable of the stability and evolution of ice shelves and it is therefore imperative to have precise estimation of it.  
...
\begin{itemize}
    ...
    \item \textbf{Paragraph 4 and 5} - discuss our approach and why it is needed: Emulator from Doury extended to SMB. Because "easier" on temperature, need to change it for SMB. It also needs to be faster than RCM. So add attention, NRMSE and DSC to speed it up and make it better. 
    \begin{itemize}
        \item We faced several challenges when extending their RCM-emulator from emulating TAS over Western Europe to SMB over Antarctica. The first problem was that SMB is known to be complex to model because, compared to temperature, it varies highly across multiple scales of space and time. Moreover, as stated by Lenaerts et al., SMB is impacted by complex interactivity between the atmosphere and the ice sheet surface, large-scale atmosphere circulations, and ice sheet topography~\cite{Lenaerts2019}. To address the difficulty of modeling SMB, we equipped the U-Net used to emulate TAS with attention mechanisms and a normalized RMSE loss designed to handle different ranges in SMB values across our physical domain. 
    ...
    \begin{comment}
    \item \textbf{Paragraph 6} - sixth paragraph will introduce your thesis statement. Think how you can distill the essence of your thesis into a single sentence. \end{comment}
