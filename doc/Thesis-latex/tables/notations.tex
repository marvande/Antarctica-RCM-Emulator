\begin{table}[!tbp]
    ...
    \toprule
        ...
        \\
        $\operatorname{LR}$ & Low-resolution (GCM resolution) & \\
        $\operatorname{HR}$ & High-resolution (RCM resolution) & \\
        $\hat{F}_U(\operatorname{UPRCM})$ & Emulator trained on UPRCM data, prediction on UPRCM & $\llbracket 1, K \rrbracket \times \llbracket 1, L \rrbracket$
        \\
        $\hat{F}_U(\operatorname{GCM})$ & Emulator trained on UPRCM data, prediction on GCM & $\llbracket 1, K \rrbracket \times \llbracket 1, L \rrbracket$
        \\
        $\hat{F}_G(\operatorname{GCM})$ & Emulator trained on GCM data, prediction on GCM & $\llbracket 1, K \rrbracket \times \llbracket 1, L \rrbracket$
        \\
